...
SELECT vets.name, animals.name, date FROM visits
JOIN vets
ON vets.id = vet_id
JOIN animals
...
SELECT COUNT(animals.name) AS number_visits, animals.name FROM visits
JOIN animals ON animals.id = animal_id
JOIN vets ON vets.id = vet_id
...
SELECT animals.name, vets.name, visits.date FROM animals
...
SELECT COUNT(*) AS un_related_visits FROM visits
JOIN vets ON visits.vet_id = vets.id
JOIN animals ON visits.animal_id = animals.id
JOIN specialization ON visits.vet_id = specialization.vet_id
JOIN species ON species.id = animals.species_id
WHERE animals.species_id <> specialization.species_id;


-- What specialty should Maisy Smith consider getting? Look for the species she gets the most
SELECT  species.name, count(species.name) AS visited_group
FROM VISITS
JOIN animals ON animals.id = visits.animal_id
JOIN species ON species.id = animals.species_id
WHERE vet_id = 2 GROUP BY species.name ORDER BY visited_group DESC LIMIT 1;